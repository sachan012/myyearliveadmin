        ...
        if ($this->form_validation->run() !== FALSE) {
            ...
            if ($user) {
              ...
              if($user["status"] != "active")
                ...
                // compare passwords
                if (!in_array( $user['role_type'], array(1,2,3))) {
            ...
            if ($user)
             {
                ...
                if ($this->admin_model->_update('admins', array('updated_at' => set_local_to_gmt(), 'password' => $NewPassword), array('email' => $emailid)))
                 {

                        $subject = "Maadima : PASSWORD RECOVERY EMAIL";
                       ...
                       /* $mailContent = "<p>Dear ".ucwords($name)." <br /> <br /> you have succesfully reset your password.</p>
                        ...
                        $mailTo = $emailid;
                        $this->sendMail($subject, $mailContent, $mailTo, ADMIN_NOTIFICATION_EMAIL, ADMIN_NOTIFICATION_TITLE); // forsending pdf to client or admin use
